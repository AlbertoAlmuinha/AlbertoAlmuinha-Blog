...
## Baseline model and Workflowsets 📝
...
Once we have our models, the next step is to cross them with the two preprocessing recipes to create the workflowsets object(we do not use the other two because we would have to modify the code because it would fail to convert the ids into dummy variables). The models are then calculated through the `modeltime_fit_workflowsets()` function (new functionality that allows integrating the Modeltime and Workflowsets package). We will do it in parallel so that the result takes less time to execute using six cores:
...
## Global Models: XGBoost and Prophet + Catboost ⚙️📊
